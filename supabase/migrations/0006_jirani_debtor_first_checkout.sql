...
begin
  ...
  if v_balance > 0 then
    ...
    if v_customer is null then
      ...
      if v_customer is null then
        ...
        values (v_user, v_org, v_customer_name, 0, 0, 'active') returning id into v_customer;
      end if;
    else
  ...
  returning public.sales.id into v_sale;
  ...
  if v_paid > 0 then
    ...
    values (v_org, v_customer, v_sale, 'sale', v_paid, case when v_payment = 'credit' then 'cash' else v_payment end, v_user, v_key || ':payment');
  end if;
